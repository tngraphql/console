...
import { Container, ContainerContract } from './Container'
...
export class Kernel implements KernelContract {
  ...
  public application: ContainerContract = new Container()

  constructor (application?: ContainerContract) {
    if (application) {
  ...
  public async runCommand (commandInstance: CommandContract, argv: string[]) {
    ...
    const response = await this.application.call(commandInstance, 'handle', [])
  ...
  public async runDefaultCommand () {
    ...
    const commandInstance = this.application.make(
